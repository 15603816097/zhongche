...
from config import HORIZON, LGB_PARAMS, MODEL_DIR, TARGET_COLUMNS
...
def train():
    print("=" * 70)
    print("LightGBM 训练：连续时序标签 + 防泄漏验证 + 最终全量重训")
    print("=" * 70)
    ...
    with open(MODEL_DIR / "model_lgb.pkl", "wb") as f:
        pickle.dump(model_final, f)

    print(f"保存: {MODEL_DIR / 'model_lgb.pkl'}")
    print(f"保存: {MODEL_DIR / 'scaler.pkl'}")
    print(f"保存: {MODEL_DIR / 'val_pred_lgb.npz'}")

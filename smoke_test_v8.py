...
def main():
    ...
    print("=" * 90)
    print("V8 online inference smoke test")
    print("=" * 90)
    ...
    if int(config.get("version", -1)) != 8:
        raise RuntimeError("当前 ensemble_config 不是 V8，请先 python activate_v8.py")
    ...
    for run in range(1, 4):
        ...
        if not np.all(np.isfinite(pred)):
            raise RuntimeError("V8 输出存在 NaN/Inf")
        ...
        print(
            ...
            f"pca={timings.get('pca', 0.0):.3f}s "

...
def main():
    ...
    if not CANDIDATE_PRED_PATH.exists() or not CANDIDATE_CONFIG_PATH.exists():
        raise FileNotFoundError(
            "缺少 V7 candidate，请先运行 python find_best_weight_v7.py"
        )
    ...
    print("\n" + "=" * 104)
    print("V7 final candidate summary")
    print("=" * 104)
    ...
    print(f"HF gains                : {candidate_config.get('v7_highpass_gains')}")
    print(f"HF smooth windows       : {candidate_config.get('v7_highpass_windows')}")
    ...
    print("\n当前线上 ensemble_config.pkl 没有被 V7 修改。")

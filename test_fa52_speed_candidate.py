...
API_PORT = int(os.getenv("API_PORT", "8821"))
CALLBACK_PORT = int(os.getenv("CALLBACK_PORT", "8897"))
N_REQUESTS = int(os.getenv("N_REQUESTS", "50"))
WAIT_TIMEOUT = float(os.getenv("WAIT_TIMEOUT", "600"))
BASELINE_PATH = Path(
    os.getenv(
        "BASELINE_PATH",
        "artifacts/fa52_speed/baseline_predictions.npz",
    )
)
...
def sync_predict(name: str, history: list[dict]) -> np.ndarray:
    payload = {
        "requestId": f"FA52_SPEED_SYNC_{name}_{API_PORT}",
    ...
    if arr.shape != (96, 6) or not np.isfinite(arr).all():
        raise RuntimeError(f"bad prediction shape/values: {arr.shape}")
    return arr
...
def main() -> int:
    if not BASELINE_PATH.is_file():
        raise SystemExit(f"missing baseline artifact: {BASELINE_PATH}")

    z = np.load(BASELINE_PATH, allow_pickle=False)
    names = [str(x) for x in z["names"].tolist()]
    baseline = np.asarray(z["predictions"], dtype=np.float64)
    histories = build_histories()

    if names != [x[0] for x in histories]:
        raise SystemExit(f"baseline names mismatch: {names}")

    health = requests.get(
        f"http://127.0.0.1:{API_PORT}/health",
        timeout=10,
    ).json()
    print("candidate health:", health)
    if (
        health.get("status") != "ok"
        or health.get("version") != "3.0.0-full-arch"
        or health.get("ensemble_version") != 8
        or health.get("model_loaded") is not True
    ):
        raise SystemExit("candidate is not frozen Full Architecture V1")

    print("\n[1/2] Exact Full Architecture prediction parity")
    max_diff = 0.0
    for i, (name, history) in enumerate(histories):
        cand = sync_predict(name, history)
        diff = float(np.max(np.abs(baseline[i] - cand)))
    ...
    try:
        for i in range(N_REQUESTS):
            ...
            rid = f"FA52_SPEED_{i:03d}_{name}"
            payload = {
                "requestId": rid,
                "history_length": len(history),
                "forecast_horizon": 96,
        ...
        print("\n" + "=" * 100)
        print("FA52-SPEED CANDIDATE SUMMARY")
        print("=" * 100)
        ...
        print(f"FA52-SPEED TEST: {'PASS' if passed else 'FAIL'}")

...
OUTPUT_PATH = MODEL_DIR / "v83_http_stress.json"

API_HOST = os.getenv("V83_STRESS_API_HOST", "127.0.0.1")
API_PORT = int(os.getenv("V83_STRESS_API_PORT", "18884"))
CALLBACK_HOST = os.getenv("V83_STRESS_CALLBACK_HOST", "127.0.0.1")
CALLBACK_PORT = int(os.getenv("V83_STRESS_CALLBACK_PORT", "18885"))
NUM_REQUESTS = int(os.getenv("V83_STRESS_REQUESTS", "50"))
SUBMIT_WORKERS = int(os.getenv("V83_STRESS_SUBMIT_WORKERS", "10"))
WAIT_SECONDS = float(os.getenv("V83_STRESS_WAIT_SECONDS", "240"))
...
def _submit_one(history: np.ndarray, k: int) -> dict:
    request_id = f"V83_STRESS_{k:03d}"
...
def _validate_callback(body: dict) -> tuple[bool, str, str | None]:
    ...
    if not isinstance(request_id, str) or not request_id.startswith("V83_STRESS_"):
...
def main() -> int:
    ...
    print("=" * 108)
    print("V8.3 REAL HTTP ASYNC + CALLBACK STRESS")
    print("=" * 108)
    ...
    candidate_ok = bool(
        ...
        and candidate.get("version") == "2.11.0-v83-final"
        and candidate.get("offline_gate_pass") is True
        and int(candidate.get("base_version", -1)) == 8
        and "acoustic_db" not in set(candidate.get("enabled_targets", []))
    )
    print(f"health before       : {'PASS' if health_before_ok else 'FAIL'}")
    print(f"candidate endpoint  : {'PASS' if candidate_ok else 'FAIL'}")
    print(f"candidate body      : {candidate}")
    ...
    expected_ids = {f"V83_STRESS_{k:03d}" for k in range(NUM_REQUESTS)}
    ...
    print(f"V8.3 REAL HTTP STRESS GATE         : {'PASS' if gate else 'REJECT'}")
